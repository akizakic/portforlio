# 📘 포트폴리오

<!-- 여기에 본인 사진을 추가하세요 -->
<img width="591" height="787" alt="image" src="https://file.notion.so/f/f/b137de05-ef23-4845-b33d-0e244f9ccc63/a13583ef-0d77-4f58-9379-ff872466e1e8/%EC%96%91%EC%B1%84%EC%9D%80_%EB%B0%98%EB%AA%85%ED%95%A8.jpg?table=block&id=2a69b3f3-8968-8147-ab21-e6972a314667&spaceId=b137de05-ef23-4845-b33d-0e244f9ccc63&expirationTimestamp=1762704000000&signature=XB9LEIMOxMy_Adr2r4SLIB4qFS4a-waESEeWdqOYGKo&downloadName=%EC%96%91%EC%B1%84%EC%9D%80_%EB%B0%98%EB%AA%85%ED%95%A8.jpg" />

---

## 💡 모두를 연결하는 든든한 다리,  
### 서버 엔지니어 **양채은**입니다.

안녕하세요.  
양채은입니다.  
새로운 도전을 즐기며, 밝은 에너지로 팀을 감싸며  
아름다운 인터페이스를 만드는 것을 좋아합니다.

---

## 📞 Contact

- **T** : 010-4927-6206  
- **A** : 서울특별시 영등포구  
- **E** : akizakic@naver.com  

---

## 🏫 Education

**인천대학교 ⎜ 정보통신공학과**  
*(부전공 : 미래자동차연계전공)*  
2021.03 - 2025.02 (졸업 예정)  
📊 학점 : 4.0 / 4.5  

---

## 📄 Certificate

- **2025.09** Certified Kubernetes Administrator (CKA)  
  발급처 : —  
  유효기간 : —  

- **2024.09** AWS Certified Solutions Architect - Associate  
  발급처 : —  
  유효기간 : —  

- **2024.06** 정보처리기사  
  발급처 : —  
  유효기간 : —  

- **2024.09** 리눅스마스터 2급  
  발급처 : —  
  유효기간 : —  

- **2023.10** SQL 개발자 (SQLD)  
  발급처 : —  
  유효기간 : —  

---

## 🛠️ Stacks

- Kubernetes  
- Linux  
- AWS  
- Docker  
- Nginx  
- Git / GitHub  

---

## 💻 Work Experience

**2025.01 - 현재**  
**더존비즈온 ⎜ TAUnit (인프라운영Cell)**  

- **검색 엔진 최적화 (SEO)**  
  - 검색엔진 최적화의 중요성을 논의하기 위한 기술 공유회 진행  
  - 다이나믹 메타태그, 시멘틱 태그, 사이트맵, JSON-LD 등 기술적 지원  
  - **성과:** 6개월 만에 유입 0 → **월 유입 1만명**으로 증가  
  - **사용 기술:** Next.js, Google Search Console, 블랙키위  

---

## 🚀 Projects

### 🎮 CJ 올리브네트웍스 클라우드웨이브 3기  
**하이퍼캐주얼 게임 제작 프로젝트 (2022.05 ~ 2022.07)**  

- Unity를 사용한 하이퍼캐주얼 게임 제작  
- 2D / 3D 두 가지 타입의 게임 기획, 제작 및 배포 경험  
- **성과:** 실제 유저 10명 확보  
- 🔗 [관련 프로젝트 보기](https://www.notion.so/2a69b3f38968816fb32df788b0d8d995?pvs=21)

---

## 👣 Activities

- **학과 학생회 활동**  
  - 다양한 행사 기획 및 운영 경험  
  - 팀워크 및 협업 능력 강화  

🔗 [활동 기록 보기](https://www.notion.so/2a69b3f3896881089adaf0d3f2b12af7?pvs=21)

---

> 🌱 “서버 엔지니어는 단순히 시스템을 운영하는 사람이 아니라,  
> 모두를 연결하는 든든한 다리가 되어야 한다고 믿습니다.”
